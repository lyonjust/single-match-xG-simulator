...
home_shots = st.text_input('Please enter the xG of all home team shots, separated by a comma (",")')
away_shots = st.text_input('Please enter the xG of all away team shots, separated by a comma (",")')
...
df_match_outcomes = pd.DataFrame(match_outcomes.T, columns=['home_goals', 'away_goals', 'home_margin'])
...
df_match_outcomes['match_outcome'] = np.select(condlist=conditions, choicelist=choices, default='Draw')

df_grouped = df_match_outcomes[['match_outcome', 'home_goals']].groupby('match_outcome').count()
