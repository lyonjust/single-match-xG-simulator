...
for i, x in enumerate(rows_home):
    x.number_input(f"xG of home shot # {i}",min_value=0.01, max_value=0.99, step=0.01, key=i)

for i, x in enumerate(rows_away):
    x.number_input(f"xG of away shot # {i}",min_value=0.01, max_value=0.99, step=0.01, key=i)